...
@click.command('release', help="Release an app (internal to the Dokos team)")
@click.argument('app')
@click.argument('bump-type', type=click.Choice(['major', 'minor', 'patch', 'stable', 'prerelease']))
@click.option('--from-branch', default='staging')
...
def release(app, bump_type, from_branch, to_branch, owner, prerelease, prerelease_date, repo_name, remote, dont_frontport):
	...
	release(bench_path='.', app=app, bump_type=bump_type, from_branch=from_branch, to_branch=to_branch, remote=remote, owner=owner, prerelease=prerelease, prerelease_date=prerelease_date, repo_name=repo_name, frontport=frontport)
