...
def get_app(name, git_url=None, branch=None, bench_path='.', skip_assets=False, verbose=False,
		...
		if res.ok:
			...
			for project in data:
				if name in [project['name'], project['path']]:
					git_url = f'https://gitlab.com/dokos/{name}'
	...
	logger.log(f'getting app {name}')
	shallow_clone = '--depth 1' if check_git_for_shallow_clone() else ''
	branch = f'--branch {branch or "master"}' if branch else ''
...
Do you want to continue and overwrite it?'''):
	...
	exec_cmd(f"git clone -q {git_url} {branch} {shallow_clone} --origin upstream {name}",
